...
<div class="space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:justify-between md:items-end gap-6">
        <div>
            <h2 class="font-serif text-4xl font-bold text-dark-brown">Ekosistem Akun</h2>
            <p class="text-gray-500 mt-2 text-lg">Kelola pengguna dan pemilik kafe.</p>
        </div>
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            <div class="flex items-center gap-8 mr-4 bg-white/50 px-6 py-3 rounded-2xl border border-white">
                <div>
                    <p class="text-gray-500 font-medium text-xs uppercase tracking-wider mb-1">Total Pengguna</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 md:gap-8 gap-6">
        @forelse ($users as $user)
            <div x-data="{ confirmDelete: false }" class="bg-white rounded-[2rem] p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/40 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                ...
                <h3 class="font-serif text-2xl font-bold text-dark-brown leading-tight flex items-center gap-2">
                    ...
                    @if($user->status === 'pending')
                        <span class="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-full text-[10px] font-bold uppercase tracking-wider">Menunggu</span>
                    @elseif($user->status === 'rejected')
                        <span class="px-2 py-1 bg-red-100 text-red-700 rounded-full text-[10px] font-bold uppercase tracking-wider">Ditolak</span>
                    @endif
                </h3>
                ...
                <div class="mt-8 pt-6 border-t border-light-beige/30 flex justify-between items-center text-sm">
                    <span class="text-gray-400 font-medium">Bergabung {{ $user->created_at?->format('M Y') ?? '—' }}</span>
                    <div class="flex gap-2 items-center">

                        <a href="{{ route('accounts.show', $user->id) }}" class="text-dark-brown font-bold uppercase tracking-wider text-xs hover:text-soft-green transition-colors ml-2">
                            Lihat Profil &rarr;
                        </a>
                    </div>
                </div>

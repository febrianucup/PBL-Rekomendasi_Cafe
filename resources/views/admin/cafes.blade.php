...
<div class="space-y-10">
    ...
    <div class="flex justify-between items-end">
        <div>
            <h2 class="font-serif text-4xl font-bold text-dark-brown">{{ __('messages.the_collection') }}</h2>
            <p class="text-gray-500 mt-2 text-lg">{{ __('messages.the_collection_desc') }}</p>
        </div>
        <button class="bg-dark-brown text-white px-6 py-3 rounded-full font-medium shadow-lg shadow-dark-brown/30 hover:bg-dark-brown/90 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
            {{ __('messages.add_new_destination') }}
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/40 relative overflow-hidden">
            ...
            <p class="text-gray-500 font-medium mb-2 relative z-10">{{ __('messages.active_listings') }}</p>
            <h3 class="font-serif text-5xl font-bold text-dark-brown relative z-10">142</h3>
        </div>
        <div class="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/40 relative overflow-hidden">
            <div class="absolute -right-6 -top-6 w-32 h-32 bg-cream rounded-full opacity-50"></div>
            <p class="text-gray-500 font-medium mb-2 relative z-10">{{ __('messages.average_rating') }}</p>
        ...
        <div class="bg-dark-brown rounded-3xl p-8 shadow-[0_8px_30px_rgb(75,46,43,0.15)] relative overflow-hidden text-white">
            ...
            <p class="text-light-beige font-medium mb-2 relative z-10">{{ __('messages.curation_status') }}</p>
            <h3 class="font-serif text-4xl font-bold relative z-10 mt-1">{{ __('messages.excellent') }}</h3>
            <p class="text-sm text-light-beige/70 mt-2 relative z-10">+12 {{ __('messages.this_week') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
        <div class="px-8 py-6 border-b border-light-beige/30 flex justify-between items-center bg-white/50 backdrop-blur-sm">
            <h3 class="font-serif text-2xl font-bold text-dark-brown">{{ __('messages.destinations') }}</h3>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-gray-400 text-sm font-medium border-b border-light-beige/30 bg-cream/30 uppercase tracking-wider">
                        ...
                        <th class="px-8 py-4 font-medium">{{ __('messages.location') }}</th>
                    ...
                    <tr class="hover:bg-cream/20 transition-colors group">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-500 text-xs mt-0.5 font-medium">{{ __('messages.added') }} {{ $cafe->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-5 text-right flex justify-end gap-2">
                            <form action="{{ route('admin.cafes.destroy', $cafe->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this cafe?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-soft-red hover:text-red-700 transition-colors font-medium">{{ __('messages.delete') }}</button>
                            </form>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-8 py-10 text-center text-gray-500 font-medium">{{ __('messages.no_cafes_found') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>

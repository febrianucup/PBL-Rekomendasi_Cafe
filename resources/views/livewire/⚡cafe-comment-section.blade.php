<div class="space-y-10 w-full h-full">
    ...
    <div class="flex gap-4 mb-6 border-b border-stone-200">
        <button type="button" wire:click="$set('commentType', 'review')" class="pb-2 font-bold {{ $commentType === 'review' ? 'border-b-2 border-black' : 'text-stone-400' }}">{{ __('messages.comment_tab_review') }}</button>
        <button type="button" wire:click="$set('commentType', 'discussion')" class="pb-2 font-bold {{ $commentType === 'discussion' ? 'border-b-2 border-black' : 'text-stone-400' }}">{{ __('messages.comment_tab_discussion') }}</button>
    </div>
        ...
        @if($isDiscussion || ($isReview && !$isOwner && !$isAdmin && !$hasReviewed))
            <form wire:submit.prevent="submitComment">
                @if($commentType === 'review')
                    <span class="text-sm font-semibold text-gray-700">{{ __('messages.rating') }}:</span>
                ...
                <textarea wire:model="body" class="w-full min-h-[120px] p-4 border border-stone-200 rounded-xl" placeholder="{{ $commentType === 'review' ? __('messages.write_review_placeholder') : __('messages.write_discussion_placeholder') }}"></textarea>
                ...
                <div class="flex justify-between items-center mt-3">
                    ...
                    <button type="submit" class="px-6 py-2 bg-black text-white rounded-xl text-xs font-bold">{{ __('messages.send') }}</button>
                </div>
        ...
        <div wire:key="comment-{{ $comment->id }}" class="p-4 bg-stone-50 rounded-xl space-y-3 border border-stone-100">
            ...
            @if(($commentType === 'review' && Auth::id() === $cafe->user_id) || $commentType === 'discussion')
                <button type="button" class="text-blue-500 text-xs font-semibold hover:underline" wire:click="toggleReply({{ $comment->id }})">
                    {{ $replyingToId == $comment->id ? __('messages.cancel') : __('messages.reply') }}
                </button>

                @if($replyingToId == $comment->id)
                    <div class="mt-3 pl-4 border-l-2 border-stone-300">
                        <form wire:submit.prevent="submitReply({{ $comment->id }})">
                            <textarea wire:model.blur="reply_body" class="w-full p-2 text-sm border border-stone-200 rounded-lg" placeholder="{{ __('messages.write_reply_placeholder') }}"></textarea>
